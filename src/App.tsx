...
function detectLanguage(code: string): QueryLanguage | null {
  ...
  if (trimmed.includes(';') || trimmed.includes('from') || trimmed.includes('select')) {
    return 'sql';
  }
...
export default function App() {
  ...
  const [schemaPresetName, setSchemaPresetName] = useState<string>('Blogging & Team Portal');
  ...
  const handleTranslate = () => {
    ...
    try {
      if (fromLanguage === 'prisma') {
        const { sql, ast, baseModel } = translatePrismaToSql(inputCode, dialect, schema);
        ...
        const { prisma, ast } = translateSqlToPrisma(inputCode, dialect, schema);
  ...
  const addHistoryItem = (input: string, output: string) => {
    ...
    if (history.length > 0 && history[0].input === input && history[0].output === output) {
      return;
    }
  ...
  const updateFieldName = (modelIdx: number, fieldIdx: number, newName: string) => {
    const next = { ...schema };
    next.models[modelIdx].fields[fieldIdx].name = newName;
    setSchema(next);
  };

  const updateFieldType = (modelIdx: number, fieldIdx: number, newType: string) => {
    const next = { ...schema };
  ...
  const updateFieldRelationModel = (modelIdx: number, fieldIdx: number, modelName: string) => {
    const next = { ...schema };
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'convert' && (
            ...
            >
              ...
              <div className="lg:col-span-9 flex flex-col gap-6" id="editors-container">
                <QueryEditor
                  fromLanguage={fromLanguage}
                  autoDetect={autoDetect}
                  inputCode={inputCode}
                ...
                {conversionResult?.success && (
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6" id="analysis-split-view">
