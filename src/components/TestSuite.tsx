...
const TEST_CASES: TestCase[] = [
  ...
  // 2. Prisma: Compound Operators
  {
    ...
    category: 'Prisma',
    description: 'Filter posts containing "Prisma" with complex author and comment exclusion constraints',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
  ...
  // 3. Prisma: E-Commerce Customers with Order Items
  {
    ...
    category: 'Prisma',
    description: 'Query Customer list with status-specific nested Orders and Items',
  ...
  // 4. Prisma: Pagination and Sorting
  {
    ...
    category: 'Prisma',
    description: 'Pagination testing using take, skip, and multi-field object ordering',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `prisma.user.findMany({
  where: {
  ...
  // 5. Prisma: Simple Primary Key Lookup
  {
    ...
    category: 'Prisma',
    description: 'Direct unique query mapping to a single SQL record with a select projection',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
  ...
  // 6. Prisma: StartsWith & EndsWith Wildcards
  {
    ...
    category: 'Prisma',
    description: 'Translates startsWith, endsWith, and contains filters to LIKE statements',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
  ...
  // 7. Prisma: Empty Filters / All Records
  {
    ...
    category: 'Prisma',
    description: 'Testing translator execution with empty criteria, pulling entire table schemas',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `prisma.post.findMany({})`
  },
  ...
  // 8. SQL: Multi-Table Join Projections
  {
    ...
    category: 'SQL',
    description: 'Retrieve users joined with author posts and specific comment texts',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `SELECT
  u.id,
...
OFFSET 0;`
  ...
  // 9. SQL: Strict Filters with Wildcards
  {
    ...
    category: 'SQL',
    description: 'Translates standard SQL LIKE conditions back to safe Prisma objects',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `SELECT
  posts.id,
...
ORDER BY posts.id ASC;`
  ...
  // 10. SQL: Customer & Orders Join
  {
    ...
    category: 'SQL',
    description: 'Complex SQL statement linking customers, nested orders, and totals',
    schema: ECOMMERCE_SCHEMA,
    schemaLabel: 'E-commerce Schema',
    input: `SELECT
  c.id,
...
WHERE o.status = 'DELIVERED' AND o.total_amount > 150.00
ORDER BY o.total_amount DESC;`
  ...
  // 11. SQL: Group By & Count Warnings
  {
    ...
    category: 'SQL',
    description: 'Validate complex SQL statements with multiple filter expressions',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `SELECT
  u.id,
...
WHERE u.role = 'AUTHOR' AND (u.id > 10 OR u.id < 5);`
  ...
  // 12. SQL: Single record projection
  {
    ...
    category: 'SQL',
    description: 'Translates exact ID matching statement into single item retrieval',
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `SELECT id, email_address FROM users WHERE id = 101 LIMIT 1;`
  },
  // 13. SQL: Multi-column pagination
  {
    ...
    schema: BLOG_SCHEMA,
    schemaLabel: 'Blogging Schema',
    input: `SELECT * FROM comments WHERE post_id = 99 ORDER BY id DESC LIMIT 10 OFFSET 20;`
  }
];
...
interface TestSuiteProps {
  onLoadTest: (input: string, category: 'Prisma' | 'SQL', schema: QuerySchema) => void;
}

export default function TestSuite({ onLoadTest }: TestSuiteProps) {
  const [selectedCaseId, setSelectedCaseId] = useState<string>(TEST_CASES[0].id);
  const [dialect, setDialect] = useState<DbDialect>('postgresql');
  const [testResults, setTestResults] = useState<Record<string, TestResult>>({});
  const [isRunningAll, setIsRunningAll] = useState<boolean>(false);

  const activeCase = TEST_CASES.find(c => c.id === selectedCaseId) || TEST_CASES[0];

  const runTestCase = (testCase: TestCase): TestResult => {
    const startTime = performance.now();
    try {
      if (testCase.category === 'Prisma') {
        const { sql, baseModel } = translatePrismaToSql(testCase.input, dialect, testCase.schema);
        ...
        const { prisma, ast } = translateSqlToPrisma(testCase.input, dialect, testCase.schema);
        const baseName = ast.properties?.table || 'user';
        const analysis = analyzeQueryStatic(testCase.input, baseName, testCase.schema);
        const endTime = performance.now();
        return {
          caseId: testCase.id,
          success: true,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="test-suite-workspace">
      {/* LEFT COLUMN: Test Cases Navigation & Overall Status */}
      <div className="lg:col-span-4 flex flex-col gap-4">
        {/* Bulk Action Panel */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-xl flex flex-col gap-4" id="bulk-tests-panel">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Layers className="text-teal-400" size={18} />
              <h2 className="font-bold text-slate-200">Query Compiler Matrix</h2>
            </div>
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            Verify the offline parser against a matrix of nested Prisma structures, boolean criteria, join models, text operators, and SQL syntax patterns.
          </p>

          <div className="grid grid-cols-2 gap-2 mt-1">
            <div className="flex flex-col bg-slate-950 p-2.5 rounded-lg border border-slate-800">
              <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Compiler Engine</span>
              <span className="text-xs font-bold text-slate-300 mt-0.5">Offline AST Parser</span>
            </div>
            <div className="flex flex-col bg-slate-950 p-2.5 rounded-lg border border-slate-800">
              <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Avg Parse Time</span>
              <span className="text-xs font-bold text-emerald-400 mt-0.5">&lt; 1 ms</span>
            </div>
          </div>

          {/* Dialect selector */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-slate-400 font-medium">Target DB Dialect for SQL compilation:</label>
        ...
        {/* Test Cases List */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col gap-2 max-h-[500px] overflow-y-auto custom-scrollbar" id="test-list-container">
      ...
      <div className="lg:col-span-8 flex flex-col gap-4" id="test-details-panel">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className={`text-xs font-extrabold px-2 py-0.5 rounded uppercase ${
                  activeCase.category === 'Prisma' ? 'bg-teal-500 text-slate-950' : 'bg-emerald-500 text-slate-950'
                }`}>
                  {activeCase.category} Case
                </span>
                <span className="text-xs text-slate-400 font-semibold">• Schema: {activeCase.schemaLabel}</span>
              </div>
              <h2 className="text-base font-bold text-slate-100">{activeCase.name}</h2>
              <p className="text-xs text-slate-400 leading-normal">{activeCase.description}</p>
            </div>

            <div className="flex items-center gap-2 shrink-0 self-end md:self-auto">
              <button
                onClick={() => onLoadTest(activeCase.input, activeCase.category, activeCase.schema)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
                  {activeCase.category === 'Prisma' ? <Code2 size={13} className="text-teal-400" /> : <Terminal size={13} className="text-emerald-400" />}
                  <span>Input {activeCase.category === 'Prisma' ? 'Prisma Code' : 'Raw SQL'}</span>
                </div>
                <span className="text-[10px] text-slate-500 font-mono">AST Input</span>
              </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
                  {activeCase.category === 'Prisma' ? <Terminal size={13} className="text-emerald-400" /> : <Code2 size={13} className="text-teal-400" />}
                  <span>Compiled {activeCase.category === 'Prisma' ? 'SQL Output' : 'Prisma Output'}</span>
                </div>
                <span className="text-[10px] text-slate-500 font-mono">Parser Outcome</span>
              </div>
              <div className="bg-slate-950 border border-slate-800 rounded-lg p-3.5 font-mono text-xs overflow-x-auto min-h-[220px] max-h-[300px] text-teal-300 relative select-all">
                {activeResult.success ? (
                  <div className="whitespace-pre-wrap leading-relaxed">{activeResult.translated}</div>
                ) : (
          ...
          {activeResult.success && (
            <div className="bg-slate-950/60 rounded-xl p-4 border border-slate-800 flex flex-col gap-3" id="test-analytics">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div className="p-3 bg-slate-900/80 rounded-lg border border-slate-800/80 flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Verification</span>
                ...
                <div className="p-3 bg-slate-900/80 rounded-lg border border-slate-800/80 flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Compilation Time</span>
                ...
                <div className="p-3 bg-slate-900/80 rounded-lg border border-slate-800/80 flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Join Recommendations</span>
                ...
                <div className="p-3 bg-slate-900/80 rounded-lg border border-slate-800/80 flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Index Alerts</span>
          ...
          {/* Protip */}
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl flex gap-3 items-start" id="test-tip">
            <Lightbulb className="text-amber-400 shrink-0 mt-0.5 animate-pulse" size={15} />
            <div className="flex flex-col gap-0.5">
              <span className="text-xs font-bold text-slate-300">Dynamic Testing Guidance</span>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Click <strong>"Load in Workspace"</strong> to modify fields, tweak models, test MySQL vs Sqlite compilation, or render the interactive visual AST Nodes inside the main compiler.
              </p>
            </div>
          </div>
        </div>
